...
// Function to send an email when an alert is triggered
  const sendAlertEmail = (email, city, temp, threshold) => {
    // Send the email
    ...
    emailjs.send('service_dejquy9', 'template_z7wopfp', templateParams, 'uPdinCV_PtwQM8eOM')
      .then((response) => {
  ...
  const triggerAlert = (email, city, temp, threshold) => {
    // Call sendAlertEmail when the alert condition is met
    sendAlertEmail(email, city, temp, threshold);
  };

  // Function to check and send alerts for each configured alert (Example trigger)
  const checkAlerts = () => {
    const triggeredAlerts = alerts.filter(alert => {
      if (currentWeather && selectedLocation === alert.city) {
        if (currentWeather.temp > alert.threshold) {
          // Trigger email
          sendAlertEmail(alert.email, alert.city, currentWeather.temp, alert.threshold);
          return true; // Alert triggered
        }
      }

      const cityData = heatMapData.find(city => `${city.city}, ${city.country}` === alert.city);
      if (cityData && cityData.temp > alert.threshold) {
        // Trigger email
        sendAlertEmail(alert.email, alert.city, cityData.temp, alert.threshold);
        return true; // Alert triggered
      }
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-lg text-gray-600">By Zohair Ahmed </h2>
            </div>
